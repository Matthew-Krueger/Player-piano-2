...
class User{
	...
	{
		$this->_db = DB::getInstance(Config::get('mysql/db/userData/name'));
	...
	public function update($fields = array(), $id=null)
	{
		...
		if(!$this->_db->update(Config::get('mysql/db/userData/users'), $id, $fields)){
	...
	public function find($user = null)
	{
		if($user){
			...
			# die();
			$data = $this->_db->get(Config::get('mysql/db/userData/users'), array($field, '=', $user));
		...
		if(!$username && !$password && $this->exists()){
			...
			if ($user) {
				if ($this->data()->password === Hash::make($password, $this->data()->salt)) {
					...
					// 	$hash = Hash::unique();
					// 	$hashCheck = $this->_db->get(Config::get('mysql/db/userData/session'), array('user_id', '=', $this->data()->id));
					//
					// 	# insert data into the stuff
					// 	if(!$hashCheck->count()){
					// 		$this->_db->insert(Config::get('mysql/db/userData/session'), array(
				...
				echo "<br><p class=\"error\">Incorrect Password</p>";
			}
			echo "<br><p class=\"error\">User does not exist!</p>";
		}
		return false;
	}
	...
	{
		$group = $this->_db->get(Config::get('mysql/db/userData/groups'), array('id', '=', $this->data()->group));

		if ($group->count()) {
			$permissions = json_decode($group->first()->perm, true);
	...
	{
		$this->_db->delete(Config::get('mysql/db/userData/session'), array('user_id', '=', $this->data()->id));
